...
## 155. Section 199C: Deterministic Revision Comparison API

**One read-only endpoint, no new persistence, and no judgement.** `GET /trips/{trip_id}/revisions/compare?left_revision_id=...&right_revision_id=...` compares two real, snapshot-backed revisions of the same trip and returns `ItineraryRevisionComparison` (`app/models/itinerary_revision_comparison.py`). The route is declared before `/revisions/{revision_id}` so `compare` is never captured as a revision id.

- **Shared diff logic, not a second implementation.** `targeted_regeneration_diff_builder.compute_itinerary_content_diff(before, after)` was extracted from the 197C targeted-regeneration diff; `build_targeted_regeneration_diff` now delegates to it (behaviour unchanged, its existing suite still passes). The comparison service reuses the same function, so items are matched by stable `experience_id` only -- never by name, coordinates, or fuzzy similarity.
- **What is compared:** added/removed experiences, experiences moved between days, within-day reorderings, traveler pace/interests, validation status, warning count, critical-issue count. `no_compared_differences` speaks only about those fields -- it is never a claim that two revisions are identical in every stored detail.
- **Names for display** are resolved from the snapshot that contains the id (added -> right, removed -> left, moved/reordered -> right then left). Direction is always left -> right.
- **Refusals (never a partial or reconstructed result):** unknown revision or a revision of another trip -> `REVISION_NOT_FOUND` (404); either revision has no stored snapshot -> `REVISION_SNAPSHOT_UNAVAILABLE` (409). The service is read-only: comparing changes no revision, branch, active branch, or `PlanningState`.
- **No winner.** The response deliberately has no score, ranking, winner, or recommendation field.
- **Branch display context.** `GET /trips/{id}/branches` responses gained optional `head_version_label`, `head_branch_display_name`, `base_version_label`, `base_branch_id`, `base_branch_display_name`, resolved server-side from the branch's own stable head/base revision ids. A fresh fork's inherited head/base is owned by its source branch, so the labels name that owner ("Main · v1"), never the fork. These are display-only; ids stay authoritative.

**Files changed**: `app/models/itinerary_revision_comparison.py` (new), `app/services/itinerary_revision_comparison_service.py` (new), `app/services/targeted_regeneration_diff_builder.py`, `app/schemas/itinerary_lineage.py`, `app/api/routes/trips.py`. **Tests**: 10 service-level + 6 API-level (`test_itinerary_revision_comparison_service.py`, `test_itinerary_revision_comparison_api.py`) covering identical revisions, added/removed/moved/reordered, cross-branch comparison, wrong-trip id, unavailable snapshot, and read-only behaviour.

## 156. Section 202B.1: Honest Regeneration, Stable Identity, Destination Resolution, AI Failure Taxonomy

Fixes the correctness/honesty defects the 202A baseline measured. Item quality (ranking, interest coverage, empty-day filling, narrator prose) is deliberately NOT touched -- that is 202B.2/202B.3.

**Legacy regeneration root cause (Tasks 1-3).** `POST /regenerate` (legacy path) -> `derive_pending_affected_stages` -> `apply_regeneration_mutation` -> `PlanningOrchestrator.rerun_affected_stages` reruns deterministic stage services over the SAME `trip_request`. No stage service reads `feedback_history`; the keyword classifier only chooses *which stages to rerun*. So "Remove X" / "make day 2 less packed" / "more local food" could never change the plan, yet the mutation created a version and marked the feedback applied because "the pipeline reran" was being treated as "the feedback was applied" (202A: 0 of 9 requested changes happened; every experience id was also regenerated).

**New legacy semantics.** Legacy regeneration runs only for a `feedback_type` registered in `regeneration_mutation_service.LEGACY_SUPPORTED_OPERATIONS`, each with a postcondition `(before, after, event) -> bool` proving the requested operation happened. The registry is intentionally EMPTY (no keyword-classified type has a mechanism that guarantees an effect; inventing one from keyword guesses is out of scope). Consequences: free text is refused with `REGENERATION_FEEDBACK_NOT_INTERPRETABLE` (409) *before* anything is rerun -- no version, no revision, feedback stays pending (`applied_at`/`applied_in_version` null), a truthful blocked `RegenerationAttempt`, no async job started. The same gate is enforced at the shared `apply_regeneration_mutation` boundary (sync route and async job runner); a registered operation whose postcondition fails raises `RegenerationNoEffectError`, the pre-rerun state is restored, and the outcome is `REGENERATION_NO_EFFECT`. Free-text changes require the targeted AI path.

**Targeted success requires an observable effect.** `TargetedRegenerationApplicationService._commit` derives affected/preserved days from the FINAL states (`partition_days_by_final_equality`: a day is preserved only if its complete `DailyPlan` is byte-identical before vs after), not from planned metadata. 202A: an additive request reported `affected=[]`/`preserved=[1,2,3]` while day 2 had received the new place, because the plan's values were reported. If nothing changed (no day, no traveler profile) the result is `NO_EFFECT` -> `REGENERATION_NO_EFFECT`: no version, no revision, feedback still pending. The version's `preserved_sections` use the truthful set too.

**Stable identity (Tasks 5-7).** Audit: `ExperienceItem.experience_id` was a fresh random id on every construction, and `provider_place_id`/`provider_source` were populated only for AI-promoted candidates. Every planner rerun therefore re-id'd unchanged places (fake Removed+Added in diffs/comparisons), and every consumer keyed on provider identity (targeted executor reserved-candidate set and duplicate audit, feedback-interpretation request builder, plan builder preserved ids) saw nothing for ordinary places. Rule (`app/services/experience_identity.py`): the stable place key is `build_candidate_id(provider_source, provider_place_id)`; every provider-backed experience now carries that identity, and its `experience_id` is `experience_<sha256(trip_id|place key)[:32]>` -- deterministic per trip, so an unchanged place keeps the same id across regenerations and branches. Never derived from names/coordinates; items without provider identity keep random ids. `compute_itinerary_content_diff` additionally compares by that key (`place:<key>` else `id:<experience_id>`), so older persisted revisions with random ids do not produce fake pairs; moved items are reported under their after-id. Verified on real data: after a targeted removal, unchanged places kept 8/9 ids (only the removed one differs); 202A legacy runs kept 0%.

**Duplicate prevention and detection (Tasks 10-11).** Root cause: `_reserved_candidate_ids` and the final duplicate audit both read `experience.provider_source/provider_place_id`, which was empty for ordinary places, so scoped reasoning could "steal" places already on preserved days and no check fired (202A: Pelourinho de Lisboa on days 1,2,3). With identity populated those checks now work. Prevention: `TargetedRegenerationExecutor._enforce_unique_places` (after each preserved-day splice) drops a repeated provider place from the NON-preserved day and records it in that day's warnings; an additive request for an already-scheduled place inserts nothing (then `NO_EFFECT`). Validator safety net: a CRITICAL `duplicate_experience` issue when the same stable place key is scheduled more than once -- reported, never silently dropped.

**Destination resolution (Tasks 12-15).** Old rule: any 3+ char ASCII word of the query appeared in the display name. It rejected "Lisbon" (display "Lisboa, Portugal") and "Cordoba" ("Córdoba" split into "c"+"rdoba" by an ASCII tokenizer), yet accepted any result -- even a business in another country -- sharing one word. New rule (`_is_plausible_geocode_result`, Nominatim requested with `addressdetails`, `namedetails`, `accept-language=en`): (1) the feature category must be `place`/`boundary`, never an amenity/shop/tourism POI; (2) the first query segment must be token-compatible (subset either way, after NFKD/diacritic/case normalization) with a name the PROVIDER reports for that feature (name, `namedetails` incl. `name:en`/`int_name`, address city/town/..., display first segment) -- exonyms are accepted only when the provider lists them, no alias table; (3) each further query segment of 4+ letters must match a provider address/display component (wrong-country context is rejected), while 1-3 letter abbreviations ("US", "DC") are unverifiable and neither confirm nor contradict. A result with no structural fields falls back to the display-name rule. Real check: "Lisbon" now plans 9 places (Jerónimos, Belém, ...); "Cordoba, Spain" resolves to Córdoba, Spain (its plan is then empty for a different reason -- candidate quality, 202B.2); bare "Cordoba" resolves to Nominatim's top match, Córdoba, Argentina -- provider ranking, not something to hardcode (disclosing the resolved place to the user is a later item). Failure UX: when the provider reports (structured `ProviderResponse.failure_reason == "destination_unresolved"`, never inferred from zero candidates) that the destination cannot be resolved, `DestinationContext.destination_resolution="unresolved"`, the validator's critical issue is the actionable `destination_unresolved` message, `POST /generate` returns `422 DESTINATION_UNRESOLVED` (the blocked state is still persisted), and an async generate job fails with the same code.

**AI/provider failure taxonomy (Tasks 16-20).** `app/providers/ai_failure.py`: `AIProviderFailureKind` = `not_connected | authentication | rate_limited | timeout_or_network | malformed_output | provider_error`, classified only from structured exception attributes (HTTP status, provider error `code`, exception type) -- never by parsing message text. All ten adapter exception sites (Groq/Anthropic x interpreter, reasoning x2, proposal, narrator) now emit a fixed, secret-free sentence instead of `f"... {exc}"` (202A found raw provider bodies -- account ids, truncated model output -- stored in user-reachable messages). `AIFeedbackInterpretationResult.failure_kind` carries the kind. Targeted outcomes: `rate_limited` -> runtime `RATE_LIMITED` -> `REGENERATION_PROVIDER_RATE_LIMITED`; every other AI kind (and a REJECTED result with no kind = a model answer that failed validation, `malformed_output`) -> `REGENERATION_AI_UNAVAILABLE` with per-kind copy; `REGENERATION_PROVIDER_UNAVAILABLE` now means only a requested PLACE lookup failed. All are pre-execution refusals: no mutation, no version, feedback not consumed, blocked attempt with the matching `reason_code`. The stale "regeneration engine has not been implemented" copy is gone everywhere (`REGENERATION_NOT_AVAILABLE` now says "Regeneration was not applied, so no plan changes were made."), as is the feedback preview's "No AI interpretation provider is connected." (now: "AI interpretation is not applied when feedback is captured."). Copy never promises a recovery time. Generation: an optional AI stage failing (proposal/reasoning/narrator) never reports another feature as unimplemented; deterministic fallbacks are used and the sanitized failure sentence is recorded on that stage's own report.

**Tests.** New: `test_legacy_regeneration_honesty.py` (real production registry via the `honest_legacy` marker; conftest otherwise installs a test-only permissive registry so pre-existing tests keep exercising versioning/branch/async machinery), `test_202b1_failure_semantics_http.py`, `test_experience_identity_and_diff.py`, `test_regeneration_outcome_truth_and_ai_failures.py`, `test_destination_resolution_structural.py`, plus an executor test. Updated for the new copy/identity: refusal tests, persistence, planner provenance, geocode-cache hash, Groq sanitization, application-service rejected/not_connected.

## 157. Section 202B.2: Candidate Quality, Interest Coverage, and Complete Itineraries

Content-quality pass driven by the 202A baseline. No city or place names anywhere; every rule is general and provider-tag based.

**Candidate-lifecycle audit (Task 1) -- where quality was lost.**
- *Provider (OSM adapter).* One Overpass query over `tourism=attraction|museum|gallery|viewpoint|artwork|zoo|theme_park` plus **every** `historic=*` element, `out center 20`. Overpass returns an id-ordered first-N of the union, so the pool was an arbitrary 20 elements dominated by memorials, statues, gates and artworks; landmarks were crowded out before scoring. There was no supply at all for outdoors (parks were fallback-only), food markets or nightlife. The tag set was discarded: only one flattened string `category = tourism or amenity or historic` survived (leisure/natural/man_made elements had category `None`).
- *Quality (`CandidateQualityService`).* Name/category keyword heuristics on that string; three hard-coded interest words; a small sculpture scored 0.6 (moderate), a memorial 0.2 ("administrative"); nothing structural about hospitals/offices (the DC hospital arrived as `amenity=hospital` + `historic=building`, flattened to category "hospital").
- *AI proposal.* 15 named/query ideas, `max_tokens=4000`; in 202A 5 of 18 runs were rejected: 3x `max completion tokens reached` (gpt-oss spends hidden reasoning tokens against the same budget), 1x a proposal missing a required key, 1x an empty generation (all HTTP 400 `json_validate_failed`, i.e. structural), plus one 429. Grounding/promotion were not the bottleneck.
- *Reasoning universe.* Candidates bounded to 40 by tier/score; each carried only name/coarse category/tier/score -- no normalized category, no interest evidence.
- *Planner.* `_order_candidates` matched interests by substring over name/category/address (so "food"/"outdoors" matched almost nothing); the day fill picked the nearest remaining candidate to an anchor regardless of quality; days were front-loaded (`[3,3,1,0]`), so a short supply always emptied the last day; no diversity control; no outlier control.
- *Validator.* No empty/thin/interest/concentration checks; near-identical informational warnings on every plan.

**Taxonomy (Tasks 2-5, 8, 13) -- `app/services/place_taxonomy.py`, the one classification boundary.** The adapter now keeps `NormalizedPlace.provider_tags`, a whitelist (`tourism, historic, amenity, leisure, natural, man_made, building, shop, office, place, artwork_type, memorial, attraction, zoo, wikidata, wikipedia, heritage`; never names/hours/ratings). `classify_place` returns normalized categories (`landmark, historic, museum, art_culture, architecture, park_nature, viewpoint, food_market, restaurant, nightlife, shopping, entertainment, religious, neighborhood_area, waterfront, general_attraction, low_value_object, unsuitable`) from tags, and only falls back to the flattened category *value* (itself an OSM tag value) when no tags exist -- never from a display name. *Unsuitable*: functional `amenity` (hospital, clinic, pharmacy, school, parking, bank, ...), `office`, residential/industrial `building`, utility `man_made` -- unless the provider ALSO says tourist attraction/museum/gallery/viewpoint or carries significance (wikidata/wikipedia/heritage/major historic type); a must-visit overrides. *Low-value objects*: artwork/fountain/small memorial types/plaques/gates without significance -- kept as candidates, score capped at 0.4, never hard-rejected. *Commercial galleries*: `tourism=gallery`/`shop=art` without significance, capped at 0.45 (0.7 when the traveler asked for art). *Parent/child*: only structural evidence collapses -- a `zoo=*` / `attraction=animal|exhibit` tag marks a sub-feature; sub-features in the same ~2 km cell of the same kind schedule at most one; untagged nearby places are never merged.

**Quality signals (Task 6/13).** Taxonomy category score (landmark 0.8 needs a wikipedia article, heritage designation or major historic type -- a bare `wikidata` tag is only a weak signal; museum 0.85; viewpoint/architecture/food market/entertainment 0.75; historic/general 0.7; park/waterfront/art/nightlife/religious 0.6; shopping 0.4), +0.03 per significance signal (max +0.06), +0.1 when provider metadata supports a requested interest, caps for low-value/gallery, unsuitable -> `rejected` (`unsuitable_place_type`). Documented deterministic constants; no popularity, rating or review data is used or implied, and copy says "significance signal, not a claim of being best". Scores now carry `normalized_category, categories, matched_interests, significance_signals, low_value_object, commercial_gallery, sub_feature_kind`.

**Interests (Tasks 7-12).** `canonical_interests` maps the free-text `TripRequest.interests` vocabulary (food, nightlife, outdoors, history, museum, art, architecture, shopping, plus synonyms/plurals) to canonical keys; unrecognised terms map to nothing. `INTEREST_CATEGORIES` maps each key to categories (food -> food_market/restaurant; nightlife -> nightlife/entertainment; outdoors -> park_nature/waterfront/viewpoint; history -> historic/landmark/museum; ...). A candidate's `matched_interests` are supported only by its provider-derived categories. The adapter now also returns supply for those categories (`leisure=park|garden|nature_reserve`, `amenity=marketplace|theatre|nightclub|arts_centre`). Restaurants remain day-level suggestions, never scheduled stops. *Coverage semantics:* an interest with viable (quality-approved) supply but zero scheduled matches is a plan finding (`interest_undercoverage`); an interest with zero viable supply is a provider limitation (`interest_supply_limited`, a suggestion) -- never blamed on the itinerary and never filled with a fabricated place. *AI reasoning evidence:* `ItineraryCandidateReference` carries `normalized_category` and `matched_interests`; both reasoning prompts print them per candidate and instruct the model to represent viable requested interests without breaking geography/pace, and that these fields are provider-derived, never inferred. *Deterministic fallback shares the same evidence.*

**Selection, balance, empty days (Tasks 17-25).** `_select_diverse_scheduling_set` (deterministic): must-visit, then one best match per requested interest that has eligible supply, then quality-ranked fill with diversity pressure: "diluted" candidates (low-value objects, commercial galleries for a non-art request, repeated sub-features) may use at most 25% of capacity (60% for an art-focused request) while non-diluted alternatives remain, a repeated primary category pays 0.03/repeat, sibling sub-features are never scheduled, and an isolated geographic outlier (beyond max(8 km, 2.5x the median distance to the selection's median centre)) is replaced by the best clustered non-diluted alternative unless it is the only place serving a requested interest. `_group_candidates_into_balanced_days` spreads the selection as evenly as the pace cap (relaxed 2 / balanced 3 / packed 4 -- unchanged) allows, then groups geographically (anchor + nearest) inside that size plan: 5 candidates over 3 days is `[2,2,1]`, not `[3,2,0]`. `_fill_empty_days` fills an empty day of a valid AI grouping from unused quality-eligible candidates only; rejected/low-priority candidates are never padding. Reused: existing haversine utilities; no new routing.

**Validator (Tasks 26-31) -- `plan_quality_findings.py`.** `empty_day` (warning; text distinguishes "N quality-approved candidates were left unscheduled" = selection gap from "no further approved candidates" = supply limit), `thin_day` (below 1/2/3 stops for relaxed/balanced/packed *and* viable candidates left unscheduled), `interest_undercoverage` / `interest_supply_limited`, `category_concentration` (>=50% low-value/gallery stops of >=4, with non-diluted alternatives unscheduled; "needs review", never "bad"). Duplicate-experience protection from 202B.1 is unchanged. Warning usefulness: informational restatements (all legs route-backed, accommodation/flight inventory availability) are now `suggestion`, so `warning` marks case-specific findings; readiness logic is unchanged (`blocked` only on critical).

**Destination visibility (Tasks 33-34).** The Nominatim resolution keeps the provider's address components; `describe_destination` (cached, no extra request) feeds `DestinationContext.resolved_destination` (`display_name` + city/region/country as returned, else `None` -- never inferred). The Traveler summary card shows "Resolved destination: <display name>" with a one-line hint to add city and country. Bare "Cordoba" therefore visibly resolves to Córdoba, Argentina (Nominatim's top match); "Cordoba, Spain" to Córdoba, Andalusia, Spain. No alias table; no picker (the provider returns one structured result per query; a choice UI is a later product decision).

**AI proposal robustness (Tasks 14-16, 35).** The Groq adapter makes at most ONE retry, only for a structural output failure (HTTP 400 `json_validate_failed`, or an unparsable response), with the batch halved (min 5); rate limits, auth, timeouts, provider errors and every semantic/grounding/factual rejection are never retried; the second failure reports "(after 2 attempt(s))". For `gpt-oss` models `reasoning_effort="low"` frees the token budget instead of blindly raising `max_tokens`, and the prompt asks for a few words per rationale. A live probe showed Groq returning intermittent empty generations (`failed_generation: ''`) -- exactly the failure the retry targets. Grounding boundary unchanged: the model only proposes names; provider grounding -> quality review -> promotion decide; nothing is inserted from LLM knowledge.

**Tests.** `test_itinerary_content_quality.py` (taxonomy, hospital/office rejection, must-visit override, low-value/gallery/sub-feature diversity, interest coverage and supply-limit, balanced allocation, empty-day fill, outlier rule, determinism, validator findings, adapter tags/ranking/query shape), proposal retry tests, resolved-destination API tests; older tests updated where they encoded front-loading, the old cache-hash/key sets, or the old severities.

## 158. Section 202B.3: Narrator Grounding and Deterministic Fallback

The 202A/202B baseline showed the itinerary narrator inventing descriptive claims no input field supported ("a popular spot on the waterfront", "immersive exhibits") and failing often. Growing a forbidden-word list cannot fix that, so the **contract** changed: the narrator is a summarizer of supplied structured fields, never a travel writer.

**Factual boundary (`providers/itinerary_narrator/contract.py`, shared by the Groq and Anthropic adapters).** Allowed statements: which listed places are on which day and in what order; that a place serves a requested interest *only when its own input line says so* (a provider-derived `matched_interests` match); the provider category printed beside a place; whether route data exists for a day (yes/no); and the listed limitations. The prompt body no longer contains the planner's `why_included` sentence, LLM #2 rationale/strategy prose, or any other free text that invites embellishment. The request also carries the deterministic split `interests_served` / `interests_unserved` (from `place_taxonomy.canonical_interests` and the scheduled places' `matched_interests`), and the system prompt forbids describing a merely-requested interest as covered. (A live run under the first 202B.3 contract wrote "Trip covers museums, food, nightlife" for a plan that served only museum/food; the split was added for that reason. The hardened contract could not be re-verified live -- see QA_REPORT.) Unavailable-data fields are sent as plain words, not field identifiers.

**Output guard (`services/itinerary_narrative_grounding.py`).** Day titles are normalised to deterministic "Day N". A structural check rejects narration containing capitalised, non-sentence-initial words that appear nowhere in the supplied vocabulary (place names, destination, categories); a rejected narration is treated as a failure, never shown.

**Deterministic fallback (`build_deterministic_narrative`).** When AI narration is anything other than `success` (rate limit, timeout, unavailable, malformed, validation/grounding failure, unexpected exception), `ItineraryNarrativeService` attaches a fact-only narrative built from the *final* `PlanningState` only: destination/dates/place count, requested interests served/unserved, "Visit A, then B." per day, and caveats ("Route data was not available for this day.", empty/thin day from the validator findings). It never adds an experience, never touches the itinerary or validation, and never marks generation failed. `ItineraryNarrativeReport.narrative_source` is `"ai"` or `"deterministic_fallback"`; `status` keeps describing the AI attempt. The disabled narrator (`ITINERARY_NARRATOR_ENABLED=false`, the default) still yields `not_connected` with no fallback narrative.

**Safe messages.** Only fixed, secret-free sentences reach the report message: classified provider failures (`ai_failure.classify_and_message`), "not configured", the grounding rejection, and "AI narration was unavailable, so a factual summary built from the itinerary data is shown." Model output snippets and validation dumps are never echoed (the adapters' former `{exc}` echoes were replaced with one fixed sentence).

**Nightlife semantics.** `place_taxonomy` no longer maps the `nightlife` interest to theatre/cultural venues (`entertainment`); only nightclub/pub/bar-type supply serves it, otherwise the honest `interest_supply_limited` finding applies.

**Tests.** `test_narrator_grounding_and_fallback.py` (contract/prompt content, served/unserved split, grounding guard, neutral titles, fallback built only from final state, no new experiences, failure matrix keeps plan + attaches fallback + safe message, forbidden output not echoed, unavailable narrator never marks generation failed), updated final-state and request-builder tests, and nightlife cases in `test_itinerary_content_quality.py`.

## 159. Section 202C.1A: Object Classification, Feedback Queue Order, Failure Causes

- **Single objects (`place_taxonomy`).** `object_kind_of` recognises tree / artwork / fountain / memorial / gate structurally. An isolated tree is low value (heritage/wikipedia describe only the tree); a documented small object is `notable_object` (eligible, capped at 0.5, never a LANDMARK or an architecture match unless `tourism=attraction`). Objects match only `history` (documented memorials) or `art` (artworks). Only wikipedia / heritage / a major historic type lift a score; a bare wikidata id never does. The planner hard-caps low-value objects at the diluted share and the validator reports `low_value_filler_skipped` (suggestion) rather than a selection gap. New flags: `CandidateQualityScore.notable_object`, `ExperienceItem.notable_object`.
