...
As of Step 164E, all four real adapters cache at least part of their results through `ProviderCacheStore` (see §6.6): `OpenMeteoWeatherAdapter` (Step 164B, successful forecasts), `NagerDateHolidaysAdapter` (Step 164C, successful per-year holiday lists), `FrankfurterCurrencyAdapter` (Step 164D, successful network-fetched exchange rates), and `OpenStreetMapPlacesAdapter` (Step 164E, **geocoding only** — its Nominatim destination lookup, not Overpass POI search, which remains live-only and still additionally keeps its own pre-existing in-process, no-TTL dict cache unrelated to `ProviderCacheStore`).
