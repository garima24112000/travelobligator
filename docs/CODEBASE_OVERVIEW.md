...
As of Step 164C, two of the four real adapters cache their actual results through `ProviderCacheStore` (see §6.6): `OpenMeteoWeatherAdapter` (Step 164B, successful forecasts) and `NagerDateHolidaysAdapter` (Step 164C, successful per-year holiday lists). The other two (OSM/Overpass+Nominatim, Frankfurter) do not — OSM's destination-geocode lookup remains cached in-process only, no TTL, unrelated to `ProviderCacheStore`.
...
`backend/app/storage/provider_cache_store.py` — `ProviderCacheStore` (Step 164A): a small local SQLite (`sqlite3`, stdlib-only) cache foundation keyed by `(source, query_hash)`, with TTL/expiry, `prune_expired`, and `clear_source`. `Settings.provider_cache_path`/`provider_cache_enabled` exist in config. **As of Step 164C, it is connected to two provider adapters: `OpenMeteoWeatherAdapter` (Step 164B) and `NagerDateHolidaysAdapter` (Step 164C)** — Open-Meteo caches successful weather responses under source `"open_meteo"` (`Settings.open_meteo_cache_ttl_seconds`, default 3600s TTL); Nager.Date caches successful per-year holiday responses under source `"nager_date"`, keyed by `country_code`+`year` (`Settings.nager_date_cache_ttl_seconds`, default 2592000s = 30 days TTL). For both, cache read/write failures fall back to the live provider call rather than failing retrieval. `OpenStreetMapPlacesAdapter`, `FrankfurterCurrencyAdapter`, `ProviderGateway`, and `PlanningOrchestrator` still don't import it; every other provider call still goes out live exactly as before these steps.
