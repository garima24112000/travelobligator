...
As of Step 164D, three of the four real adapters cache their actual results through `ProviderCacheStore` (see §6.6): `OpenMeteoWeatherAdapter` (Step 164B, successful forecasts), `NagerDateHolidaysAdapter` (Step 164C, successful per-year holiday lists), and `FrankfurterCurrencyAdapter` (Step 164D, successful network-fetched exchange rates). Only OSM/Overpass+Nominatim does not — its destination-geocode lookup remains cached in-process only, no TTL, unrelated to `ProviderCacheStore`.
