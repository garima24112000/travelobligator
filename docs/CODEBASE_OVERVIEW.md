...
A human-run-only smoke script, `backend/scripts/manual_provider_cache_smoke.py` (documented in `docs/21_manual_provider_cache_smoke.md`, Step 164D.1, extended in Step 164F), exercises all four cache-wired providers above against their real public APIs — including OSM/Nominatim geocoding, but never Overpass POI search. It is never imported by `app.main` and never run by pytest/CI; it requires `RUN_LIVE_PROVIDER_CACHE_SMOKE=true` and uses its own temporary cache file, never the app's real one.
