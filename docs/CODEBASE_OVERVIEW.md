...
As of Step 164B, one of the four real adapters caches its actual results: `OpenMeteoWeatherAdapter` caches successful forecasts through `ProviderCacheStore` (see §6.6). The other three (OSM/Overpass+Nominatim, Nager.Date, Frankfurter) do not — OSM's destination-geocode lookup remains cached in-process only, no TTL, unrelated to `ProviderCacheStore`.
...
`backend/app/storage/provider_cache_store.py` — `ProviderCacheStore` (Step 164A): a small local SQLite (`sqlite3`, stdlib-only) cache foundation keyed by `(source, query_hash)`, with TTL/expiry, `prune_expired`, and `clear_source`. `Settings.provider_cache_path`/`provider_cache_enabled` exist in config. **As of Step 164B, it is connected to exactly one provider adapter: `OpenMeteoWeatherAdapter`**, which caches successful weather responses under source `"open_meteo"` (`Settings.open_meteo_cache_ttl_seconds`, default 3600s TTL); cache read/write failures fall back to the live Open-Meteo call rather than failing weather retrieval. `OpenStreetMapPlacesAdapter`, `NagerDateHolidaysAdapter`, `FrankfurterCurrencyAdapter`, `ProviderGateway`, and `PlanningOrchestrator` still don't import it; every other provider call still goes out live exactly as before this step.
