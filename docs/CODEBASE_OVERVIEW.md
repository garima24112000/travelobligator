...
A human-run-only smoke script, `backend/scripts/manual_provider_cache_smoke.py` (documented in `docs/21_manual_provider_cache_smoke.md`, Step 164D.1, extended in Steps 164F and 164H), exercises all four cache-wired providers above against their real public APIs — including OSM/Nominatim geocoding and one respectful OSM/Overpass POI search (`search_attractions`, one known destination) — but never `search_restaurants`, `search_accommodation_pois`, or `search_must_visit_place`. It is never imported by `app.main` and never run by pytest/CI; it requires `RUN_LIVE_PROVIDER_CACHE_SMOKE=true` and uses its own temporary cache file, never the app's real one.
