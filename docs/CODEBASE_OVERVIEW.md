...
`generation_progress` (`GenerationProgress`, Step 163B) exists purely as backend `PlanningOrchestrator` pipeline stage-progress bookkeeping — which stage of `/generate` is running or has run — for future frontend progress polling; it's read-only via `GET /trips/{trip_id}/generation-progress` and not wired into the frontend yet.
