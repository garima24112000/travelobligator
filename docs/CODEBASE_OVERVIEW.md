...
`backend/app/storage/provider_cache_store.py` — `ProviderCacheStore` (Step 164A): a small local SQLite (`sqlite3`, stdlib-only) cache foundation keyed by `(source, query_hash)`, with TTL/expiry, `prune_expired`, and `clear_source`. `Settings.provider_cache_path`/`provider_cache_enabled` exist in config. **Not yet connected to any provider adapter** — `OpenStreetMapPlacesAdapter`, `OpenMeteoWeatherAdapter`, `NagerDateHolidaysAdapter`, `FrankfurterCurrencyAdapter`, `ProviderGateway`, and `PlanningOrchestrator` don't import it; every provider call still goes out live exactly as before this step.
