...
## 61. Section 198B: Revision UX, Async Result-Contract Parity, and Real Browser Verification

```text
feedback captured -> regenerate (readiness-gated)
  |
  v
POST /trips/{trip_id}/regenerate
  |
  +-- sync 200 (ASYNC_GENERATION_ENABLED=false, default)
  |     -> RegenerateResponseData (unchanged since 197C/198A)
  |
  +-- async 202 (ASYNC_GENERATION_ENABLED=true)
        -> StartJobResponseData -> poll GET /jobs/{job_id}
        -> completed JobResponseData now ALSO carries previous_version/
           targeted/interpretation_status/execution_status/
           affected_day_indices/preserved_day_indices/diff/
           clarification_reason/clarification_possible_experience_ids --
           the SAME TargetedRegenerationDiff model the sync route
           returns, added to GenerationJob (backend/app/models/
           generation_job.py), JobResponseData, the Postgres repository/
           DB model, and a new Alembic migration (this is the one "small
           backend change" 198B's own spec explicitly permitted)
  |
  v
buildRevisionSummaryFromSyncResponse() / buildRevisionSummaryFromJob()
  -> ONE shared `RevisionSummary` shape, ONE shared render path (Task 6)
  |
  v
trip reload (loadPlanResult, unchanged) -> revision summary + structured
diff (added/removed/moved/reordered/profile/validation) + clarification/
failure UX + version/feedback history, all reading real persisted fields
```

**Async backend contract parity (Tasks 2-5, the one permitted backend change).** Before this section, `JobResponseData` carried only `result_version`/`changed_sections` -- a completed async targeted-regeneration job had no way to expose its diff, so the frontend could only show the generic `JobStatusCard`, never the rich revision summary the sync path already had. `GenerationJob` gained 8 new optional/defaulted fields (all backward compatible -- an old, already-persisted job, or any `generate`/legacy `regenerate` job, still deserializes with `targeted=False`/empty lists/`None`, never a validation error); `JobResponseData`/`JobResponseData.from_job` mirror them; `run_targeted_regenerate_job` (`backend/app/services/generation_job_service.py`) now passes the exact same `TargetedRegenerationRuntimeResult` fields through on both its `mark_job_succeeded` and `mark_job_failed` calls, never a second hand-derived diff shape. The Postgres repository (`postgres_job_repository.py`) and `GenerationJobRow` (`app/db/models.py`) gained matching columns/serialization, plus a new Alembic migration (`6f678d2b3ed9`, chained after `6fabeaa6455e`) adding them via `op.add_column` with nullable/`server_default` semantics -- verified against a real, throwaway local Postgres container (`docker run postgres:16`, not `docker compose`, so no repo file was touched): a real `alembic upgrade head` applied cleanly, a manually-inserted pre-migration-style row read back with correct defaults (`targeted=false`, `previous_version=NULL`, `affected_day_indices=[]`), `alembic downgrade -1` removed exactly the 8 new columns and nothing else, and a subsequent `alembic upgrade head` re-applied cleanly. All 27 previously-skipped `TRAVELOB_RUN_POSTGRES_TESTS=1` integration tests were also run against this real database and passed.

**A real gap found by the Postgres round-trip itself, not by inspection**: `_row_to_job`'s naive mapping (`row.targeted`, `row.affected_day_indices`, ...) raised a `pydantic_core.ValidationError` for a `GenerationJobRow` built in-memory without going through a real `INSERT` (exactly what several existing repository tests do) -- Postgres's own `server_default` only applies at real insert time, not to a bare Python attribute access. Fixed by falling back to the same defaults `GenerationJob`'s own Pydantic fields already use (`bool(row.targeted)`, `row.affected_day_indices or []`, etc.) rather than trusting every caller to have gone through a real `INSERT` first.

**Component extraction audited (Task 1); one narrow extraction made, no broader rewrite.** `RegenerationReadinessSection`'s revision-summary/clarification/failure JSX stayed inline -- each branch reads a different field of `RevisionSummary`/`TargetedRegenerationDiff`, and all of `regenerateSuccess`/`targetedSummary`/`clarificationDetail` are set together, read together, and cleared together at the top of `handleRegenerate`, so splitting them into separate components would mean threading that same tightly-coupled state across new prop boundaries for no real readability gain. The one genuinely identical, repeated shape -- "Added"/"Removed" (heading + plain label list) -- was factored into a small `TargetedDiffCategoryList` component; "Moved"/"Reordered" were deliberately left inline since each has its own per-item shape.

**Lifecycle audit (Tasks 2/3): no bug found, backed by concrete evidence, not assumed.** Every trip-identity-changing action (`handleSelectMyTrip`, `handleLoadExistingTrip`, `handlePlanTrip`, `handleLogout`, `handleAuthenticationRequired`) already calls `setResult(null)` before fetching the next trip's data, and `RegenerationReadinessSection` is only ever rendered inside the parent's `{result && (...)}` block -- so React genuinely unmounts the whole component (discarding `regenerateSuccess`/`targetedSummary`/`clarificationDetail`/`activeJob` and every other piece of local state) on every trip switch/creation/reload/logout, rather than merely re-rendering it with a new `tripId` prop. Within one mounted lifetime, `handleRegenerate` already clears those same three pieces of state at the very top of every call (covering "subsequent regeneration" and "conflict reload", since a conflict is only ever discovered mid-call, after that same clearing already ran). The "before" snapshot (`beforeNameMap`) was never stored in React state at all -- it's a local variable recomputed fresh from the current `dailyPlans` prop on every `handleRegenerate` call and discarded when the function returns, so it cannot leak between attempts or trips by construction. This reasoning is now recorded as a code comment at the top of the component's state block so a future reader doesn't have to re-derive it.

**Diff hierarchy completed (Tasks 2, 8-24 continuation)**: two real gaps were found and fixed -- `TargetedRegenerationDiff.reordered_days` was never rendered anywhere (now shown as "Reordered: Day N" after Moved, matching the added/removed/moved/reordered order), and `critical_issue_count_before/after` was never rendered (now shown alongside the existing warning-count comparison). Empty categories were already omitted (each block is `{...length > 0 && (...)}`) -- confirmed, not changed.

**Feedback history now distinguishes Pending / "Applied in vN" / Needs clarification (Task 26), not a raw `handling_status` string.** `FeedbackEvent.applied_in_version` existed on the type already but was unused; a new `feedbackEventStatusLabel` helper reads it (plus `applied_at` and, for a genuinely AI-interpreted event, `structured_result.clarification !== null`) to render the real, already-persisted state -- never a new claim.

**Focus management on error/clarification appearance (Task 37, part of the accessibility sweep).** Both the clarification panel and the generic failure panel now share a `ref` + `tabIndex={-1}` + a `useEffect` that moves focus onto whichever one appears, using the existing `FOCUS_RING_CLASSNAME` convention -- proven live in a real browser (see Browser E2E below), not just wired and assumed.

**Lock-blocked wording added (Task 3 continuation).** `REGENERATION_BLOCKED_BY_LOCKS` previously fell through to only the generic backend `message`; it now also gets a dedicated explanatory line matching the backend's own safety contract ("never worked around... a locked plan must be unlocked first").

**A real, live-browser-caught regression, fixed (Task 5): legacy async jobs showed "None yet -> vN" instead of the true "vN-1 -> vN".** `RegenerationMutationResult.previous_version` was already computed by `apply_regeneration_mutation` and already used by the synchronous legacy route, but `run_regenerate_job` (the async legacy runner) never passed it to `mark_job_succeeded` -- invisible before this section, because `JobResponseData` had no `previous_version` field at all until Task 2-5's contract change made the gap user-visible. Fixed with one line; locked in with both a new assertion on the existing test and a live browser re-verification showing the corrected "v1 -> v2".

**A second real, live-browser-caught gap, fixed (Task 7): a clarification refusal never refreshed the main page state.** `handleRegenerationRefusal`'s `REGENERATION_NEEDS_CLARIFICATION` branch used to call a standalone `getTrip` just to populate `clarificationDetail`, never writing that fresh data back into the page's `result` -- so the feedback-history list kept showing its stale pre-attempt snapshot (still "Pending") even though the backend had already persisted a real interpretation. Now reuses the same full `onRegenerateSuccess()` refresh the `REGENERATION_CONFLICT` branch already used, so `clarificationDetail` and the rest of `result` (itinerary/version history/feedback history) move together from one fetch. Verified live: the feedback-history card now shows "Needs clarification" and the real persisted `AI interpretation` block immediately after a clarification refusal.

**Real browser E2E (Playwright, already installed in this environment from a prior verification pass -- reused, not reinstalled).** Distinguishing REAL (live backend, real generation pipeline, real deterministic feedback classification, no AI credentials involved) from CONTROLLED (a live server with `targeted_regeneration_application_service.regenerate` monkeypatched in-memory for that one process only -- the exact same seam the repo's own backend unit tests already patch -- because live Anthropic/Groq credentials are unavailable and must not be fabricated):
- **REAL**: legacy sync regeneration end-to-end (signup -> real trip -> real generation -> real feedback -> real regenerate -> v1->v2->v3 version transitions, correct changed-sections/applied-feedback-ids, "Applied in vN" rendering).
- **REAL**: legacy async regeneration end-to-end (`ASYNC_GENERATION_ENABLED=true`, `TARGETED_REGENERATION_ENABLED=false` -- real 202, real job polling, real completion, correct "v1 -> v2" after the Task 5 fix, no targeted card, no `undefined`).
- **REAL**: `TARGETED_REGENERATION_ENABLED=true` with the AI interpreter genuinely `not_connected` (no credentials used or fabricated) -- a real `REGENERATION_PROVIDER_UNAVAILABLE` refusal, with focus proven to move onto the alert panel (`document.activeElement` checked directly, after first focusing an unrelated element).
- **CONTROLLED**: async targeted success -- real 202/poll/completion, with the monkeypatched result showing `targeted=true`, `v1 -> v2`, affected/preserved days, and all four diff categories (Added/Removed/Moved/Reordered) plus profile and validation-count comparisons rendering correctly end to end.
- **CONTROLLED**: clarification -- real 202/poll/failure, the exact persisted clarification reason and possible-experience references rendering in the alert panel, focus proven to move onto it, version confirmed unchanged, feedback confirmed still pending, feedback textarea confirmed still usable.
- **Responsive**: 375px/390px/1440px all measured `scrollWidth === clientWidth` (no horizontal overflow) on a real generated trip page, both for a "blocked" (no provider data) and a fully-populated (real maps/experiences) trip.
- Not executed: a live-AI targeted success/clarification run (would require real Anthropic/Groq credentials, which are unavailable and were not fabricated) -- the CONTROLLED runs above are the strongest verification performed for those two scenarios.

**Security/privacy and no-fabrication audits (Tasks 8-9)**: the full diff was searched for secrets/API keys/session tokens/raw prompts/chain-of-thought/debug `console.*` calls -- none found (the only string matches are a test's own deliberate "does this forbidden word leak" assertions). New/changed user-facing copy was searched for `guaranteed`/`verified`/`optimal`/`improved`/`better`/`fixed`-style claims -- none found; all new copy ("Reordered: Day N", "Applied in vN", the lock-blocked explanation, critical-issue counts) is plain factual restatement of real backend fields, matching the existing "Unchanged in this revision, never guaranteed forever" convention.

**Files changed**: backend -- `app/models/generation_job.py`, `app/schemas/generation_job.py`, `app/services/generation_job_service.py`, `app/repositories/postgres_job_repository.py`, `app/db/models.py`, `alembic/versions/6f678d2b3ed9_add_targeted_regeneration_fields_to_.py`, plus new/extended tests (`test_run_targeted_regenerate_job.py`, `test_migration_schema_targeted_regeneration_fields.py`, `test_db_models.py`, `test_generation_job_service.py`). Frontend -- `frontend/lib/types.ts` (`JobResponseData` extended to mirror the new job fields), `frontend/app/page.tsx` (`RevisionSummary` type + `buildRevisionSummaryFromSyncResponse`/`buildRevisionSummaryFromJob`, `TargetedDiffCategoryList`, `feedbackEventStatusLabel`, reordered/critical-issue rendering, focus-management refs/effect, lock-blocked copy, the Task 5/7 fixes above). Docs -- this section.

**Verification**: backend `python -m compileall backend/app backend/scripts` clean; full `pytest` (local-JSON backend) -- 4044 passed, 18 skipped (the live-Postgres tests, which were separately run and passed against a real database, see above); frontend `tsc --noEmit`/`npm run lint`/`npm run build` all clean throughout. All temporary Playwright scripts, screenshots, and local auth-state files created during this section's browser verification were deleted afterward -- none are present in the working tree.

## 62. Section 199C: Branch Workspace UX, Revision Comparison, and Browser E2E

**Exposes the 199A/199B branching backend as a single "Itinerary branches" panel** (`frontend/app/BranchWorkspacePanel.tsx`, mounted in both Traveler and Developer view, keyed by `trip_id`). Everything it shows is read from the backend (`listBranches`, `listBranchRevisions`, `getRevisionDetail`, `createBranch`, `activateBranch`, `compareRevisions` in `lib/api.ts`, mirrored types in `lib/types.ts`) and reloaded after every action. No branch state, head, or diff is computed or faked client-side, and identity is always `branch_id` / `revision_id` -- never a display name or a `vN` label.

```text
Main (default)           -- Active: currently loaded for editing   head: Main · v2
Weekend option           -- Not active                             head: Main · v1 (inherited; no revisions of its own yet)
                            Created from Main · v1                  [Switch to Weekend option]
```

**User flow.**
1. *Create* -- "Create branch (alternate itinerary from a revision)" opens a form: name (trimmed, <= 80 chars, unique per trip case-insensitively; a name conflict is shown inline on the name field), a source-revision selector (default: the active branch's head; revisions with no stored snapshot are listed but disabled with "historical snapshot unavailable"), and an optional "Switch to this branch after creating it". Creating never changes the source revision or the current itinerary; if activation was requested but refused, creation and the refusal are reported separately.
2. *Switch* -- "Switch to <branch>" restores that branch's head snapshot. It creates no revision. After success the trip is reloaded from the backend, feedback panel state is reset, and the regeneration section is remounted (`workspaceEpoch` key) so a "Regeneration applied" banner from the previous branch never carries over.
3. *Blocked switching* -- the backend stays authoritative. Pending feedback, active locks, and a running job are shown as an informational hint beforehand; the button is disabled only during an in-flight activation or while a regeneration (sync request or async job) is known to be running. A refusal opens a `role="alert"` panel that takes focus and says why ("Apply or resolve the pending feedback...", "Unlock the itinerary...", "Wait for the current itinerary job..."); a workspace-consistency conflict (`BRANCH_STATE_CONFLICT`) is shown as a refusal and never resolved client-side.
4. *Revisions* -- per branch, strictly branch-local (an inherited base/head is listed under its owning branch). Labels repeat across branches, so rows and options are always shown as "Branch · vN".
5. *Read-only historical preview* -- "View read-only" fetches `GET /revisions/{id}` and shows the stored itinerary days/places under a focused "Historical revision -- read only" heading with only a "Back to active itinerary" action. Feedback, locks, and regeneration never apply to a preview.
6. *Compare* -- two selectors (snapshot-backed revisions only) and a factual result: added / removed / moved / reordered, pace and interests, and validation status and counts, all left -> right, with explicit empty-state copy ("No itinerary-content differences found... Other stored details are not compared"). The UI never ranks, scores, or recommends either side.

**Deferred / out of scope (unchanged from the 199 boundaries):** merging branches, deleting branches, rebase/cherry-pick, regenerating a historical preview in place, reconstructing unavailable snapshots (a metadata-only revision stays "Historical snapshot unavailable" and cannot be forked or compared), and any ranking or recommendation of branches.

**Implementation notes.** Effects avoid synchronous `setState` (lint rule `react-hooks/set-state-in-effect`): comparison defaults and stale-selection dropping are derived at render time, and the load effect fetches with a cancel flag. `RegenerationReadinessSection` gained an optional `onRunningChange` callback so the panel can disable switching during a running regeneration.

**Verification.**
- `npx tsc --noEmit`, `npm run lint`, and `npm run build` clean.
- REAL browser (Playwright/Chromium, live backend, real signup and real trip generation): initial single Main branch; fork creation without activation; activation and switch back (no Main revision created); independent regeneration on a fork; pending-feedback refusal with focus on the alert panel; name-conflict inline error.
- CONTROLLED browser (real backend over real HTTP with a throwaway process-local seam that injects labelled controlled experiences, because the sandbox has no network and the real places provider reports `unavailable`, so the real plan has zero scheduled places): moved/removed-item comparison across branches, lock-blocked switching and recovery after unlocking, content restored per branch, a slowed async regeneration disabling switching, and a metadata-only revision fixture (disabled source option, excluded from compare selectors, `REVISION_SNAPSHOT_UNAVAILABLE` from the API).
- Responsive: no horizontal overflow at 375 / 390 / 1440 px in both views with a long (80-char) branch name, expanded revision lists, the create form, a preview, and a comparison result open. Accessibility: semantic headings, labelled controls (no unnamed controls), active state stated in text, `role="status"` live region, keyboard activation of a switch button, focus moved to error panels and the preview heading.

## 63. Section 202B.1: Structured Failure Mapping (Frontend)

No layout change. `page.tsx` maps the backend's new structured codes (never message text) in `REGENERATION_REASON_CODE_LABELS`: `REGENERATION_FEEDBACK_NOT_INTERPRETABLE` ("Request needs AI interpretation"), `REGENERATION_NO_EFFECT` ("Nothing to change"), `REGENERATION_PROVIDER_RATE_LIMITED` ("AI temporarily rate-limited"), `REGENERATION_AI_UNAVAILABLE` ("AI interpretation unavailable"), `BRANCH_STATE_CONFLICT`, `DESTINATION_UNRESOLVED`; `REGENERATION_PROVIDER_UNAVAILABLE` keeps meaning "a requested place could not be looked up" (its hint text is unchanged and now accurate). The four honest-refusal codes add one line under the backend's own truthful message: nothing was changed, no new version was created, feedback is still pending. Feedback history still shows "Applied in vN" only from `applied_at`/`applied_in_version`, which a refusal never sets. Clarification keeps its own amber panel. The generation error banner shows the backend's actionable `DESTINATION_UNRESOLVED` message.

## 64. Section 202B.3: Traveler Polish, Reload Persistence, Mobile and Accessibility

**Traveler vs Developer boundary.** Traveler view shows only what helps someone decide about their trip; anything a developer needs to debug the pipeline stays in Developer view (the backend fields are untouched; only presentation is gated by `mode`). Audit of the old Traveler view: *A (kept)* destination/dates/request, itinerary by day, limitations, feedback text and status, regeneration outcome, branches, weather/stay/flight summaries; *B (developer, now Developer-only)* raw trip id, pipeline status, candidate counts, feedback type/affected stages/regeneration strategy/change preview, "Applied feedback" event ids, coordinate flags, provider enum names, the "load a trip by ID" panel; *C (evaluation)* unchanged in Developer view.

**Central display labels (`lib/display-labels.ts`).** One map for provider/source names (`openstreetmap_places` -> "OpenStreetMap", `osrm` -> "OSRM routing", ...), `placeDataLabel` ("Place data: OpenStreetMap"), `routingDataLabel` ("Routing data unavailable"), and `travelerText`, which rewrites internal fragments in backend sentences (`ai_candidate_promotion_report`, `destination_context.candidate_restaurants`, "haversine", "via osrm") and humanizes any leftover snake_case token so an identifier never reaches a traveler. Developer view does not use it.

**Warning severity.** `TravelerLimitationsSection` ("Important limitations", `TravelerSections.tsx`) groups the validation report as Critical / Warning / Suggestion, distinguished by text label, symbol and colour (never colour alone). Near-identical findings (same severity, category and message modulo digits) collapse to one row "(applies to N items)"; per-day narrator caveats that repeat on every day are stated once under the trip summary. Critical items and warnings are never collapsed; only optional suggestions sit behind a disclosure button. Developer view still lists every finding.

**Hierarchy (Traveler).** destination heading (the provider-resolved `display_name`, with "You entered: ..." when it differs and the hint "If this isn't the place you meant, include the city and country when creating the trip.") and dates/request -> itinerary by day -> important limitations -> feedback and regeneration -> branches -> supporting detail (weather/holidays/currency, where to stay, flights) as accessible disclosures (`aria-expanded`/`aria-controls`, Enter/Space; a jump link to `#id` opens its section).

**Narrator display.** A `deterministic_fallback` narrative is shown with the fixed sentence "AI narration was unavailable, so a factual summary built from the itinerary data is shown." (the provider/HTTP detail is Developer-only); its per-day "Visit A, then B" line is omitted in Traveler view because the stop list sits directly below.

**Reload persistence.** Root cause of "reload returns to home": the open trip lived only in React state (`result`), so a reload started from an empty page. The smallest mechanism is a URL hint, `?trip=<trip_id>` (`lib/trip-selection.ts`, `history.replaceState`); no PlanningState, plan content, token or identity is stored anywhere. After sign-in the app reads the hint once per session and opens the trip through the same authenticated fetch as a My-trips click, so branch, version, itinerary, feedback, pending state, locks and readiness all come from the backend. A 401/403/404/not-generated response clears the hint. The hint is cleared on logout, on session loss, at the start of a new plan, and when a plan fails, so a later sign-in (possibly another person) never reopens it. Explicit behavior: reload with a live session -> same trip; logout then log in again (or reload after logout) -> home with My trips, no trip open; a link with `?trip=` for someone else's trip -> the backend refuses and the hint is dropped. Frontend-only state is not restored (read-only historical preview, comparison selection, half-typed forms); logout also clears the create form, regeneration-running state and workspace epoch.

**Mobile.** One CSS rule (`globals.css`, `max-width: 640px` or coarse pointer) gives buttons, selects, inputs, textareas, nav links and role=button elements a 44px minimum; map links and Leaflet zoom controls are 44px; card padding is `p-4 sm:p-5`; the app shell padding is `px-3 sm:px-6`. Leaflet stop markers are non-interactive (no fake `role="button"`, no per-marker tab stop). The 202A audit found 51 controls under 32px at phone width; at 375/390 px the measured count under 44px is now 6, all third-party Leaflet/OSM attribution links, which are inline text links.

**Accessibility.** Day headings are `h3`; disclosure controls are real buttons; severity has a text label; live regions (`role="status"`/`"alert"`) are unchanged; the resolved destination has an `sr-only` sentence; a duplicate `id="branches"` (wrapper plus panel) and an invalid `<dl>` child were fixed. axe-core (4.10, run in the browser against the production build) reports 0 violations in both Traveler and Developer view after the `<dl>` fix (it had flagged one invalid `<dl>` child in Developer view).

**Loading (sync and async).** The old panel advanced a timer-driven fake percentage and rotated invented stage sentences. It now shows only real signals: backend stage progress when reported, otherwise an indeterminate pulse, plus an honest elapsed-seconds counter ("Working for 23s. Real provider lookups can take a minute or two; the request is still running."). The submit button is disabled while a request runs (one `POST /generate`). An async job failure shows its own structured message once (no duplicate error box); the failed trip stays listed under My trips and the reload hint is cleared. A reload while an async job is running reopens the trip and resumes polling.

**Async release-mode verification** (`ASYNC_GENERATION_ENABLED=true`, process-level, against `next start` production build): create -> 202 job -> polling -> completion -> trip loaded -> Main branch initialised -> reload restores the same trip; `DESTINATION_UNRESOLVED` shows the actionable message once. See QA_REPORT "202B.3 follow-up" for REAL vs CONTROLLED labelling.

**`BranchWorkspacePanel.tsx` audit (Task 17).** It is already its own file (~1.1k lines) separate from `page.tsx`; a further split (comparison / preview / create-form sub-components) was considered and not done: the panel is one cohesive state machine, there is no frontend test framework to protect a refactor, and 199C behavior (focus management, live regions, refusal panels) was verified in a real browser and is unchanged.

## 65. Section 202C.1A: Pending-Feedback Queue Labels

`FeedbackPanel` (both views) labels pending requests **Next to apply** / **Waiting · #N in line** using `pending_feedback_summary.queue_event_ids` (re-derived from `created_at`, stable sort, for older payloads), shows an explainer (one saved request per Regenerate, oldest first; waiting requests are never skipped, reordered or deleted) and, for the next item, "Last attempt: AI temporarily rate-limited / interpretation unavailable. Nothing was changed; this request is still saved." from the recorded regeneration attempts. Applied items keep "Applied in vN". The Regenerate hint states it applies the oldest saved request first. Nothing is reordered or removed client-side.
