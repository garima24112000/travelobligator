...
# Step 185C: Expedia-labeled local/manual accommodation parser. Mirrors
# `booking.py` exactly -- see that module's docstring for the full
# design rationale (synthetic `data-source="expedia"` micro-format,
# delegates to generic for untagged cards, no live fetch, no browser
# automation). Expedia is restricted from live scraping under this
# project's policy (see `app.providers.scraping_source_registry_
# defaults`'s `expedia` entry).

DATA_SOURCE = "expedia"
PARSER_VERSION = "accommodation_source_parser_expedia_v1"
