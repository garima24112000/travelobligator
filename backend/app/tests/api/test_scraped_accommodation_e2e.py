...
def test_full_generate_with_scraped_local_still_succeeds_without_langgraph(
    ...
    level by test_generation_progress.py/backend/app/tests/graphs/
    test_langgraph_planning_graph.py -- this just confirms the full
    generate flow still succeeds end to end with scraped_local enabled,
    i.e. enabling it doesn't change that."""
    html_path = _write_html(tmp_path, _TEST_HTML_ONE_PROPERTY)
    _enable_scraped_local(monkeypatch, html_path)
