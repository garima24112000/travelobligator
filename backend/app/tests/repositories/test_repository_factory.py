...
def test_factory_module_import_does_not_connect() -> None:
    ...
    assert hasattr(module, "get_job_repository")
...
def test_get_job_repository_returns_local_singleton_by_default(
    ...
    assert factory_module.get_job_repository() is local_job_repository
...
def test_factory_returns_postgres_job_repository_when_selected(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    """Step 186F: selecting postgres never connects on its own --
    constructing PostgresJobRepository only builds a lazy engine/session
    factory (see app/db/session.py)."""
    ...
    repo = factory_module.get_job_repository()

    assert isinstance(repo, PostgresJobRepository)
    # Cached: a second call while still selected as postgres returns the
    # exact same instance, matching get_engine()'s established pattern.
    assert factory_module.get_job_repository() is repo
