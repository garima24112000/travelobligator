"""Opt-in Postgres-backed trip repository (Step 183D).
...
same way rather than rejecting a duplicate.
"""
...
class PostgresTripRepository:
    ...
    def create(self, trip_id: str) -> TripRecord:
        ...
        stmt = stmt.on_conflict_do_update(
            ...
            set_={"status": "draft", "created_at": now, "updated_at": now},
        )
        with self._session_factory() as session:
            session.execute(stmt)
            session.commit()

        return TripRecord(trip_id=trip_id, status="draft", created_at=now, updated_at=now)
